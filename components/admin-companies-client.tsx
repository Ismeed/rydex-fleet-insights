...
export function AdminCompaniesClient({
  ...
  return (
    <AppShell
      title="Tenant Companies Registry"
      description="Manage SaaS sub-accounts, billing subscriptions, and workspace configurations"
      user={user}
    ...
    >
      ...
      <section className="bg-white border border-border rounded-xl p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h3 className="font-bold text-base text-foreground">Registered Tenant Companies</h3>
                  ...
                  <tr key={c.id} className="hover:bg-surface/30 transition-colors">
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <div className="size-8 bg-brand/10 text-brand font-bold rounded-lg flex items-center justify-center uppercase shrink-0">
                          {c.name.charAt(0)}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="font-semibold text-foreground truncate block">{c.name}</span>
                          <span className="text-[10px] text-muted-foreground truncate">{c.email || "No email"} • {c.phone}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-semibold text-brand bg-brand-soft/45 px-2 py-0.5 rounded-full w-max">
                          {c.fleetType || "General"}
                        </span>
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={cn(
                          "px-2.5 py-1 text-[9px] font-bold rounded-full uppercase tracking-wider",
                          c.status === "ACTIVE"
